...
namespace CityInfo.API.Services
{
    public class CityInfoRepository : ICityInfoRepository
    {
        ...
        public async Task UpdateTicketForPointOfInterest(int cityId, int pointOfInterestId)
        {
            var city = await GetCityAsync(cityId, true);
            if(city != null)
            {
                city.PointsOfInterest.Where(c => c.Id == pointOfInterestId).FirstOrDefault().SoldTickets++;
            }
        }
